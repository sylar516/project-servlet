...
public class LogicServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        // получаем значение флага, если true, то игра заканчивается и отправляется редирект на index.jsp
        boolean endGame = (boolean) req.getSession().getAttribute("endGame");
        if (endGame) {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (checkWin(resp, currentSession, field)) {
            //Меняем значение флага, так как игра закончена
            currentSession.setAttribute("endGame", true);
            return;
        }
